...
for i = 1:6
    ...
    legend_entries = cell(length(valores_f2), 1); % Crear una celda para las entradas de la leyenda
    
    for j = 1:length(valores_f2)
        % Graficar la variable correspondiente
        plot(tvalores_ori_f2{j}, yvalores_ori_f2{j}(:, i+1));
        ...
        legend_entries{j} = sprintf('f_2 = %g\n', valores_f2(j));
    end
    ...
    legend(legend_entries{1:length(valores_f2)}); % Mostrar solo las entradas con valores asociados
